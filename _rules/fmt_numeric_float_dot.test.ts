...
import { ruleData } from "./fmt_numeric_float_dot.ts";
...
Deno.test("Valid 2", { permissions: "none" }, () => {
	const diagnostics = Deno.lint.runPlugin(rule, "foo.ts", `const foo = 2;`);
	deepStrictEqual(diagnostics.length, 0);
});
...
Deno.test("Valid 4", { permissions: "none" }, () => {
	const diagnostics = Deno.lint.runPlugin(rule, "foo.ts", `const foo = -4;`);
	deepStrictEqual(diagnostics.length, 0);
});
