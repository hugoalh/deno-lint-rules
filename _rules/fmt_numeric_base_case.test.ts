...
import rule from "./fmt_numeric_base_case.ts";
...
Deno.test("BigInt Valid 1", { permissions: "none" }, () => {
	const diagnostics = Deno.lint.runPlugin(plugin, "foo.ts", `const foo = 0b101n;`);
	deepStrictEqual(diagnostics.length, 0);
});
Deno.test("BigInt Valid 2", { permissions: "none" }, () => {
	const diagnostics = Deno.lint.runPlugin(plugin, "foo.ts", `const foo = 0o43n;`);
	deepStrictEqual(diagnostics.length, 0);
});
Deno.test("BigInt Valid 3", { permissions: "none" }, () => {
	const diagnostics = Deno.lint.runPlugin(plugin, "foo.ts", `const foo = 0xCDn;`);
	deepStrictEqual(diagnostics.length, 0);
});
...
Deno.test("Number Valid 1", { permissions: "none" }, () => {
	const diagnostics = Deno.lint.runPlugin(plugin, "foo.ts", `const foo = 0b101;`);
	deepStrictEqual(diagnostics.length, 0);
});
Deno.test("Number Valid 2", { permissions: "none" }, () => {
	const diagnostics = Deno.lint.runPlugin(plugin, "foo.ts", `const foo = 0o43;`);
	deepStrictEqual(diagnostics.length, 0);
});
Deno.test("Number Valid 3", { permissions: "none" }, () => {
	const diagnostics = Deno.lint.runPlugin(plugin, "foo.ts", `const foo = 0xCD;`);
	deepStrictEqual(diagnostics.length, 0);
});
