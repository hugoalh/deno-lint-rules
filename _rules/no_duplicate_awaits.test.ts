...
import rule from "./no_duplicate_awaits.ts";
...
Deno.test("Valid 1", { permissions: "none" }, () => {
	const diagnostics = Deno.lint.runPlugin(plugin, "foo.ts", `await doSomething();`);
	deepStrictEqual(diagnostics.length, 0);
});
Deno.test("Valid 2", { permissions: "none" }, () => {
	const diagnostics = Deno.lint.runPlugin(plugin, "foo.ts", `await (await doSomething()).doAnotherSomething();`);
	deepStrictEqual(diagnostics.length, 0);
});
