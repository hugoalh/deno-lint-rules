...
import rule from "./fmt_hex_case.ts";
const plugin = constructPlugin({
	[rule.identifier]: rule.querier()
});
Deno.test("BigInt Invalid 1", { permissions: "none" }, () => {
	const sample = `const foo = 0x34cdn;`;
	const diagnostics = Deno.lint.runPlugin(plugin, "foo.ts", sample);
	deepStrictEqual(diagnostics.length, 1);
	deepStrictEqual(sample.slice(...diagnostics[0].range), "34cd");
});
Deno.test("BigInt Valid 1", { permissions: "none" }, () => {
	const diagnostics = Deno.lint.runPlugin(plugin, "foo.ts", `const foo = 0x34CDn;`);
	deepStrictEqual(diagnostics.length, 0);
});
Deno.test("Number Invalid 1", { permissions: "none" }, () => {
	const sample = `const foo = 0x34cd;`;
	const diagnostics = Deno.lint.runPlugin(plugin, "foo.ts", sample);
	deepStrictEqual(diagnostics.length, 1);
	deepStrictEqual(sample.slice(...diagnostics[0].range), "34cd");
});
Deno.test("Number Valid 1", { permissions: "none" }, () => {
	const diagnostics = Deno.lint.runPlugin(plugin, "foo.ts", `const foo = 0x34CD;`);
	deepStrictEqual(diagnostics.length, 0);
});
Deno.test("String X Invalid 1", { permissions: "none" }, () => {
	const sample = `const foo = "\\xa9";`;
	const diagnostics = Deno.lint.runPlugin(plugin, "foo.ts", sample);
	deepStrictEqual(diagnostics.length, 1);
	deepStrictEqual(sample.slice(...diagnostics[0].range), "a9");
});
Deno.test("String X Valid 1", { permissions: "none" }, () => {
	const diagnostics = Deno.lint.runPlugin(plugin, "foo.ts", `const foo = "\\xA9";`);
	deepStrictEqual(diagnostics.length, 0);
});
...
Deno.test("String U Invalid 1", { permissions: "none" }, () => {
	...
	const diagnostics = Deno.lint.runPlugin(plugin, "foo.ts", sample);
	deepStrictEqual(diagnostics.length, 1);
	deepStrictEqual(sample.slice(...diagnostics[0].range), "00a9");
});
Deno.test("String U Invalid 2", { permissions: "none" }, () => {
	const sample = `const foo = "\\ud87e\\udc04";`;
	const diagnostics = Deno.lint.runPlugin(plugin, "foo.ts", sample);
	deepStrictEqual(diagnostics.length, 2);
	deepStrictEqual(sample.slice(...diagnostics[0].range), "d87e");
	deepStrictEqual(sample.slice(...diagnostics[1].range), "dc04");
});
Deno.test("String U Invalid 3", { permissions: "none" }, () => {
	const sample = `const foo = "\\\\ud87e\\udc04";`;
	const diagnostics = Deno.lint.runPlugin(plugin, "foo.ts", sample);
	deepStrictEqual(diagnostics.length, 1);
	deepStrictEqual(sample.slice(...diagnostics[0].range), "dc04");
});
Deno.test("String U Valid 1", { permissions: "none" }, () => {
	const diagnostics = Deno.lint.runPlugin(plugin, "foo.ts", `const foo = "\\u00A9";`);
	deepStrictEqual(diagnostics.length, 0);
});
...
Deno.test("String UWrap Invalid 1", { permissions: "none" }, () => {
	const sample = `const foo = "\\u{2f804}";`;
	const diagnostics = Deno.lint.runPlugin(plugin, "foo.ts", sample);
	deepStrictEqual(diagnostics.length, 1);
	deepStrictEqual(sample.slice(...diagnostics[0].range), "2f804");
});
Deno.test("String UWrap Valid 1", { permissions: "none" }, () => {
	const diagnostics = Deno.lint.runPlugin(plugin, "foo.ts", `const foo = "\\u{2F804}";`);
	deepStrictEqual(diagnostics.length, 0);
});
