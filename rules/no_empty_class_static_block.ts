			...
			StaticBlock(node: Deno.lint.StaticBlock): void {
				...
				if (node.body.type === "BlockStatement" && node.body.body.length === 0) {
					...
					if ((context.sourceCode.text.slice(blockBegin + 1, blockEnd - 1)).trim().length === 0) {
						context.report({
							node,
							message: `Empty class static block is forbidden. While not technically errors, usually occur due to refactoring that was not completed, which cause confusion when reading code.`,
